...
import freecad.woodWorkbench.tools.generationfunctions as meshGen
...
def main(self):
    ...
    x_min,x_max,y_min,y_max,boundaries,boundary_points_original, sec_area = \
        meshGen.Clipping_Box(box_shape,box_center,box_size,box_width,box_depth,x_notch_size,y_notch_size,self.form)
    ...
    if radial_growth_rule == 'binary_lloyd':
        ...
        sites_vor, radii = meshGen.CellPlacement_Binary_Lloyd(nrings,width_heart,width_early,width_late,\
        ...
        # ---------------------------------------------
        # debug run
        sites_vor,radii = meshGen.CellPlacement_Debug(nrings,width_heart,width_early,width_late)  
    ...
    if flowFlag in ['on','On','Y','y','Yes','yes']:
        flow_nodes, flow_elems = meshGen.BuildFlowMesh(outDir,geoName,nsegments,long_connector_ratio,z_min,z_max, \
    ...
    [voronoi_vertices,finite_ridges_new,\
        ...
        meshGen.RebuildVoronoi_ConformingDelaunay_New(vor_vertices,vor_edges,ray_origins,ray_directions,\
    ...
    meshGen.LayerOperation(NURBS_degree,nsegments,theta_min,theta_max,finite_ridges_new,boundary_ridges_new,nfinite_ridge,nboundary_ridge,\
    ...
    meshGen.RidgeMidQuarterPts(voronoi_vertices_3D,nvertex,nvertices_in,voronoi_ridges,\
    ...
    [all_vertices_2D, max_wings, flattened_all_vertices_2D, all_ridges,vert_area] = \
        meshGen.VertexandRidgeinfo(all_pts_2D,all_ridges,npt_per_layer,\
    ...
    meshGen.GenerateBeamElement(voronoi_vertices_3D,nvertices_3D,NURBS_degree,nctrlpt_per_beam,nctrlpt_per_elem,nsegments,\
                        npt_per_layer,\
    ...
    if precrackFlag in ['on','On','Y','y','Yes','yes']:
        ...
        [precrack_elem,nconnector_t_precrack,nconnector_l_precrack] = \
            meshGen.InsertPrecrack(all_pts_2D,all_ridges,nridge,precrack_nodes,\
    ...
    # lp = LineProfiler()
    # lp_wrapper = lp(meshGen.ConnectorMeshFile)
    ...
    [ConnMeshData,conn_l_tangents,height_connector_t] = \
        meshGen.ConnectorMeshFile(geoName,IGAvertices,connector_t_bot_connectivity,\
                    connector_t_reg_connectivity,connector_t_top_connectivity,\
                    connector_l_connectivity,all_vertices_2D,\
    ...
    [mass,bulk_volume,bulk_density,porosity] = meshGen.ModelInfo(box_height,vert_area,sec_area)

    # ---------------------------------------------
    # Bezier extraction 
    knotVec = meshGen.BezierExtraction(NURBS_degree,nbeam_total)
    npatch = beam_connectivity_original.shape[0]

    meshGen.BezierBeamFile(geoName,NURBS_degree,nctrlpt_per_beam,\
    ...
    [nsvars_beam, nsecgp, nsvars_secgp, iprops_beam, props_beam, \
        ...
        nsvars_conn_l, iprops_connector_l, props_connector_l, props_strainrate] = \
            meshGen.ABQParams(nconnector_l,nconnector_l_precrack,nconnector_t,nconnector_t_precrack,\
    ...
    if inpType == 'abaqus':
        meshGen.AbaqusFile(geoName,NURBS_degree,npatch,nsegments,IGAvertices,beam_connectivity,\
        ...
        meshGen.ChronoMesh(geoName,IGAvertices,beam_connectivity,NURBS_degree,nctrlpt_per_beam,nconnector_t_per_beam,npatch,knotVec)
    elif inpType == 'all':
        meshGen.AbaqusFile(geoName,NURBS_degree,npatch,nsegments,IGAvertices,beam_connectivity,\
                    connector_t_bot_connectivity,connector_t_reg_connectivity,\
                    connector_t_top_connectivity,connector_l_connectivity,\
    ...
    meshGen.LogFile(geoName,outDir,mass,bulk_volume,bulk_density,porosity,vert_area,sec_area)
    ...
    if visFlag in ['on','On','Y','y','Yes','yes']:
        ...
        # lp = LineProfiler()
        # lp_wrapper = lp(meshGen.VisualizationFiles)
        ...
        # lp.print_stats()
        meshGen.VisualizationFiles(geoName,NURBS_degree,nlayers,npt_per_layer_vtk,all_pts_3D,\
                       nsegments,nridge,voronoi_ridges,all_ridges,nvertex,\
                       nconnector_t,nconnector_l,nctrlpt_per_beam,ConnMeshData,\
                       conn_l_tangents,all_vertices_2D, flow_nodes, flow_elems)  
