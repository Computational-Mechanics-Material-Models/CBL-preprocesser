...
# functions for finding the nearest line segments for points
    INFTY = 1000000
    MIN_SIZE = 5000
...
def Annualringseeds(log_center,r_max,r_min,nrings,cellsize_early,cellsize_late,
                    iter_max,print_interval):
    '''packing circles in annual rings'''
    
    # Annual ring width distribution
    width_heart = 0.3*(r_max-r_min)/nrings
    width_early = 0.7*(r_max-r_min)/nrings
    width_late = 0.1*(r_max-r_min)/nrings
    ...
    #scale = np.concatenate((np.zeros(,1),np.zeros(1)))
    radius = np.concatenate(([0],np.cumsum(width)))
    #radius = np.linspace(r_min,r_max,nrings+1) 
    ...
    for ibin in range(0,nrings*2+1):
        iter = 0 
        while iter < iter_max:
            r = (radius[ibin+1]-radius[ibin])*math.sqrt(random.random()) + radius[ibin]  # randomly generate a point in the domain
    ...
    #    if( ((vor.vertices[i,0]-log_center[0])**2+(vor.vertices[i,1]-log_center[1])**2) > (r_max+width_heart)**2 ):
...
def RebuildVoronoi(vor,circles,boundaries,log_center,x_min,x_max,y_min,y_max,box_center,boundaryFlag):
    ...
    if len(boundaries) == 3:
        for pointidx, simplex in zip(vor.ridge_points, vor.ridge_vertices):
            ...
            # print("simplex={0}".format(simplex))
    ...
    for pointidx, simplex in zip(infinite_ridges_pointid, infinite_ridges):
        # print("pointidx={0}".format(pointidx))
        simplex = np.asarray(simplex)
            
        i = simplex[simplex >= 0][0] # index of finite end Voronoi vertex of the infinite ridge
        p = vor.vertices[i]
        # print("p={0}".format(p))
        # print("point1={0}, point2={1}".format(circles[pointidx[1],0:2],circles[pointidx[0],0:2]))
        ...
        if simplex[simplex < 0][0] == -9999999999:
            ...
            # print("midpoint={0}".format(midpoint))
            for boundary in boundaries: # loop over boundary lines
                ...
                if (u >= 0) and (u <= 1):
                    if np.sign(np.dot(midpoint - log_center, normal)) == 1: # facing outwards
                        ...
                        if (t >= 0) and math.isfinite(t):
                            t_final = t
                            # print("t_final={0}".format(t_final))
        ...
        # print("intersect_point={0}".format(intersect_point))
        

        # plt.plot(intersect_point[0], intersect_point[1], 'k^')
        plt.plot([vor.vertices[i,0], intersect_point[0]], [vor.vertices[i,1], intersect_point[1]], 'k-',linewidth=1)
                
            # sol = get_solution(boundary_idx, [(midpoint[0,0],midpoint[0,1])])
            # print("sol={0}".format(sol))
            # q = sol[(midpoint[0,0],midpoint[0,1])][1][0]
            # s = np.asarray(sol[(midpoint[0,0],midpoint[0,1])][1][1]) - np.asarray(sol[(midpoint[0,0],midpoint[0,1])][1][0])
            # print("q={0}".format(q))
            # t = abs(np.cross((q-p),s)/np.cross(normal,s))
            # print("t={0}".format(t))
            # intersect_point = p + np.sign(np.dot(midpoint - log_center, normal)) * normal * t
            # print("intersect_point={0}".format(intersect_point))
            # plt.plot(intersect_point[0,0], intersect_point[0,1], 'k^')
            
            
            # plt.plot([vor.vertices[i,0], intersect_point[0,0]], [vor.vertices[i,1], intersect_point[0,1]], '--',linewidth=2)
...
def RebuildVoronoi_new(vor,circles,boundaries,log_center,x_min,x_max,y_min,y_max,box_center,merge_tol,boundaryFlag):
    ...
    voronoi_vertices_in = np.unique(voronoi_vertices_in,axis=0)
    voronoi_vertices_out = np.unique(voronoi_vertices_out,axis=0)
    # plt.plot(voronoi_vertices_in[:, 0], voronoi_vertices_in[:, 1], 'ro', markersize=1)
    # plt.plot(voronoi_vertices_out[:, 0], voronoi_vertices_out[:, 1], 'bs')
    ...
    for pointidx, simplex in zip(infinite_ridges_pointid, infinite_ridges):
        ...
        # print("intersect_point={0}".format(intersect_point))
        # plt.plot(intersect_point[0], intersect_point[1], 'k^')
        # plt.plot([vor.vertices[i,0], intersect_point[0]], [vor.vertices[i,1], intersect_point[1]], '--',linewidth=2)
    ...
    # print("voronoi_vertices_in={0}".format(voronoi_vertices_in))
    # print("nvertices_in={0}".format(nvertices_in))
    # print("boundary_points={0}".format(boundary_points))
    # print("finite_ridges_new={0}".format(finite_ridges_new))
    # print("infinite_ridges_new={0}".format(infinite_ridges_new))
    # for vpair in finite_ridges_new:
    #     plt.plot([voronoi_vertices_in[vpair[0],0], voronoi_vertices_in[vpair[1],0]],[voronoi_vertices_in[vpair[0],1], voronoi_vertices_in[vpair[1],1]], 'r--')
    ...
    while len(rows_to_fuse_voronoi_vertices_in) != 0:
        ...
        # print("rows_to_fuse_voronoi_vertices_in_whole={0}".format(rows_to_fuse_voronoi_vertices_in_whole))
        # print("voronoi_vertices_in_fused={0}".format(voronoi_vertices_in_fused))
        ...
        if len(np.unique(voronoi_vertices_in_fused,axis=0)) != len(voronoi_vertices_in_fused):
            # print("in three points loop now vertices in")
            [unique_pts,unique_inverse] = np.unique(voronoi_vertices_in_fused,return_inverse=True,axis=0)
            # print("unique_pts={0}".format(unique_pts))
            # print("unique_inverse={0}".format(unique_inverse))
            ...
            for i in range(0,len(rows_to_fuse_voronoi_vertices_in_whole)):
                ...
                # # Replace the indices in row_to_fuse_voronoi_vertices_in as well because some vertices might be replaced multiple times in each closeby point search
                # rows_to_fuse_voronoi_vertices_in[i+1:,:][rows_to_fuse_voronoi_vertices_in[i+1:,:] == rows_to_fuse_voronoi_vertices_in[i,0]] = nvertices_in + i
                
        # print("finite_ridges_merged={0}".format(finite_ridges_merged))
        # print("infinite_ridges_merged={0}".format(infinite_ridges_merged))
        voronoi_vertices_in_whole = np.vstack((voronoi_vertices_in_whole,voronoi_vertices_in_fused)) # create a whole vertex list
        # print("voronoi_vertices_in_whole={0}".format(voronoi_vertices_in_whole))
        ...
        # print("voronoi_vertices_in_new={0}".format(voronoi_vertices_in_new))
        ...
        tree_voronoi_vertices_in = cKDTree(voronoi_vertices_in)
        rows_to_fuse_voronoi_vertices_in = tree_voronoi_vertices_in.query_pairs(r=merge_tol,output_type='ndarray')
        # print("rows_to_fuse_voronoi_vertices_in={0}".format(rows_to_fuse_voronoi_vertices_in))
    ...
    # plt.plot(voronoi_vertices_in[:, 0], voronoi_vertices_in[:, 1], 'go', markersize= 4)
    
    # for vpair in finite_ridges_merged:
    #     plt.plot([voronoi_vertices_in_whole[vpair[0],0], voronoi_vertices_in_whole[vpair[1],0]],[voronoi_vertices_in_whole[vpair[0],1], voronoi_vertices_in_whole[vpair[1],1]], 'g-')
    ...
    if boundaryFlag in ['on','On','Y','y','Yes','yes']: 
        ...
        for i in range(0,boundary_points_new.shape[0]-1):
            ...
            boundary_points_new[next_point_id] = [np.inf,np.inf]
        boundary_ridges_new = (boundary_ridges_new+nvertices_in).astype(int) # shift the indices with "nvertices_in"
        # print("boundary_ridges_new={0}".format(boundary_ridges_new))
        ...
        # print("boundary_points={0}".format(boundary_points))
        # print("rows_to_fuse_boundary_points={0}".format(rows_to_fuse_boundary_points))
        # print("boundary_ridges_new={0}".format(boundary_ridges_new))
        while len(rows_to_fuse_boundary_points) != 0:
            ...
            # print("boundary_points_fused={0}".format(boundary_points_fused))
            ...
            if len(np.unique(boundary_points_fused,axis=0)) != len(boundary_points_fused):
                # print("in three points loop now boundary points")
        ...
        for i in range(0,boundary_points_featured.shape[0]):
            # print("boundary_points_featured[i,:]={0}".format(boundary_points_featured[i,:]))
            rows_to_replace_boundary_points_whole = tree_boundary_points_whole.query_ball_point(boundary_points_featured[i,:],merge_tol)
            # print("rows_to_replace_boundary_points={0}".format(rows_to_replace_boundary_points))
            # print("boundary_ridges_merged={0}".format(boundary_ridges_merged))
            if len(rows_to_replace_boundary_points_whole) != 0: 
                ...
                # plt.plot(nearby_points[:,0], nearby_points[:,1], 'r^', markersize= 3)
                # replace indices
        ...
        for i in range(0,len(boundary_points)):
            ...
            # print("boundary_points[i,:]={0}".format(boundary_points[i,:]))
            # print("rows_to_replace_voronoi_vertices_in={0}".format(rows_to_replace_voronoi_vertices_in))
            if len(rows_to_replace_voronoi_vertices_in) != 0:
                ...
                for j in range(0,len(rows_to_replace_voronoi_vertices_in_whole)):
                    ...
                    # rows_to_replace_voronoi_vertices_in_whole[j,1] = np.where(np.all(voronoi_vertices_in_whole==voronoi_vertices_in[rows_to_replace_voronoi_vertices_in[j,1],:],axis=1))[0][0]
        
                # Visualize nearby points
                nearby_points = voronoi_vertices_in_whole[rows_to_replace_voronoi_vertices_in_whole]
                # plt.plot(nearby_points[:,0], nearby_points[:,1], 'r^', markersize= 10)
                # print("rows_to_replace_voronoi_vertices_in_whole={0}".format(rows_to_replace_voronoi_vertices_in_whole))
                # replace indices
    ...
    nvertex = voronoi_vertices.shape[0]
    # nvertex = voronoi_vertices_whole.shape[0]
    ...
    if boundaryFlag in ['on','On','Y','y','Yes','yes']: 
        ...
        nvertices_in = np.unique(np.reshape(voronoi_vertices_whole[finite_ridges_merged],(-1,2)),axis=0).shape[0] - 0
        # nboundary_pts = np.unique(np.reshape(voronoi_vertices_whole[boundary_ridges],(-1,2)),axis=0).shape[0]
    ...
    #     voronoi_vertices = np.vstack((voronoi_vertices_in,boundary_points)) # vertical stack "in" vetices and "cross" boundary points
    #     nvertex = voronoi_vertices.shape[0]
    #     finite_ridges_new = np.copy(finite_ridges_merged)
    #     boundary_ridges_new = np.copy(infinite_ridges_new)
    #     nboundary_ridge = boundary_ridges_new.shape[0]
    #     voronoi_ridges = np.vstack((finite_ridges_new,boundary_ridges_new))
    #     nridge = voronoi_ridges.shape[0]
...
# def DelaunayFlowCells(voronoi_vertices,sites,):
...
#==================================================================================================
    ...
    for i in range(0,npt_per_layer): # loop over all vertices
        ...
        direction_info = []
        direction_info.append(np.where(all_ridges[allrows,0:2] == i)[1]) # append the position of current vertex in each ridge, p1 --> 0, p2 --> 1 
        ...
        vertex_info.append(all_ridge_angles[allrows]) # append wing angles
    ...
    ridge_directions = -np.power(-1,ridge_directions+1) # convert the way to express the direction of each ridge to: 1/-1 --> pointing from/to the current vertex
    ...
    outpath = Path(App.ConfigGet('UserHomePath') + '/woodWorkbench') 
    meshpath = outpath.joinpath(geoName + '-mesh.inp')
    meshf = meshpath.absolute().as_posix()
    
    meshfile = open(meshf,'a')

    np.savetxt(meshfile, all_vertices_2D, fmt='%.16g', delimiter=' '\
...
[xcoord ycoord nwings ridge1 ... ridgen farvertex1 ... farvertexn length1 ... lengthn width1 ... widthn angle1 ... anglen]', comments='')
    ...
    ridgepath = outpath.joinpath(geoName + '-ridge.mesh')
    ridgefile = ridgepath.absolute().as_posix()

    np.savetxt(ridgefile, all_ridges, fmt='%d', delimiter=' '\
    ...
    for i in range(0,nbeam_per_grain):
        for j in range(0,nconnector_t_per_beam):
            for k in range(0,nridge):
                ...
                # connector_t_connectivity[irow,:] = (voronoi_ridges[k][0]+i*npt_per_layer*nctrlpt_per_beam+j*NURBS_degree*npt_per_layer,voronoi_ridges[k][1]+i*npt_per_layer*nctrlpt_per_beam+j*NURBS_degree*npt_per_layer)
...
# Meshdata = [nodex1 nodey1 nodez1 nodex2 nodey2 nodez2 centerx centery centerz dx1 dy1 dz1 dx2 dy2 dz2 width height]    
    ...
    # for i in range(0,nelem_connector_l):
    #     for j in range(0,int(all_vertices_2D[connector_l_vertex_dict[i],2])):
    #         irow = i + j 
    #         Meshdata[irow+nelem_connector_t_bot+nelem_connector_t_reg+nelem_connector_t_top,16] = all_vertices_2D[connector_l_vertex_dict[i],3+max_wings*2+j]
        
    Meshdata[:,6:9] = (Meshdata[:,0:3] + Meshdata[:,3:6])/2
    Meshdata[0:nelem_connector_t_bot,8] = Meshdata[0:nelem_connector_t_bot,8] # if ignore the eccentricity
    # Meshdata[0:nelem_connector_t_bot,8] = Meshdata[0:nelem_connector_t_bot,8] + height_connector_t/2
    Meshdata[nelem_total-nelem_connector_t_top:nelem_total,8] = Meshdata[nelem_total-nelem_connector_t_top:nelem_total,8] # if ignore the eccentricity
    # Meshdata[nelem_total-nelem_connector_t_top:nelem_total,8] = Meshdata[nelem_total-nelem_connector_t_top:nelem_total,8] - height_connector_t/2
    ...
    # Add z-variation/random field for connector heights    
    # for i in range(0,nridge):
    #     Meshdata[i,16] = height_connector_t
    ...
    outpath = Path(App.ConfigGet('UserHomePath') + '/woodWorkbench') 
    meshpath = outpath.joinpath(geoName + '-mesh.inp')
    meshfile = meshpath.absolute().as_posix()

    np.savetxt(meshfile, Meshdata, fmt='%.16g', delimiter=' '\
...
def VisualizationFiles(geoName,NURBS_degree,nlayer,npt_per_layer_vtk,all_pts_2D,\
    ...
    conn_l_directions = flattened_all_vertices_2D[5,:]
    # get the correct angle, if conn_l_directions is -1, use the opposite direction
    # ref: https://math.stackexchange.com/questions/1319615/how-to-calculate-opposite-direction-angle
    conn_l_angles = flattened_all_vertices_2D[4,:] + np.pi*(-conn_l_directions.clip(max=0))
    ...
    # Quad_tangent[nconnector_t:nquad_conn] = rot.T*conn_l_directions[:, np.newaxis]
    # Quad_tangent[nconnector_t:nquad_conn] = np.tile(np.array([1.0,0.0,0.0]),(nconnector_l,1)) # assume a tangent of (1,0,0) for all conn_l
    ...
    # Add eccentricities to longitudinal connectors
    Quad_center[(nquad_conn_total-nconnector_t_bot):nquad_conn_total,:] = Quad_center[(nquad_conn_total-nconnector_t_bot):nquad_conn_total,:] + Quad_tangent[(nquad_conn_total-nconnector_t_bot):nquad_conn_total,:]*Quad_height[(nquad_conn_total-nconnector_t_bot):nquad_conn_total,np.newaxis]/2
...
# =============================================================================
    ...
    #woodVTKcell_types =np.ones(nvertex).astype(int)
    ...
    # woodVTKcell_types =np.concatenate((np.ones(npt_total_vtk),3*np.ones(nconnector_t),3*np.ones(nconnector_l),21*np.ones(nbeam_total_vtk))).astype(int)
    ...
    outpath = Path(App.ConfigGet('UserHomePath') + '/woodWorkbench') 
    vtkpath = outpath.joinpath(geoName + '_00001'+'.vtu')
    vtkfile = vtkpath.absolute().as_posix()

    vtkfile = open (vtkfile,'w')
    #vtkfile = open (os.path.join(vtk_path,odbname)+"_instance"+str(ins_id)+step_name+"_"+frame_name+'.vtu','w')
    ...
    vtkfile.write("<"+"PointData"\
        ...
        # +" "+"Scalars="+'"'+"collocation_flag,Temperature"+'"'+">"+'\n')
    ...
    # vtkfile.write("<"+"DataArray"+" "+"type="+'"'+"Float32"+'"'+" "+"Name="+'"'+"Temperature"+""+'"'+" "+"format="+'"'+"ascii"+'"'+">"+'\n')
    # for i in range(0,npt_total_vtk):
    #     X = 1
    #     vtkfile.write('%11.8e'%X+'\n')
    # vtkfile.write("</DataArray>"+'\n')
  		...
  		# # Displacement
  		# vtkfile.write("<"+"DataArray"+" "+"type="+'"'+"Float32"+'"'+" "+"Name="+'"'+"Spatial_displacement"+'"'+" "+"NumberOfComponents="+'"'+"3"+'"'+" "+"format="+'"'+"ascii"+'"'+">"+'\n')
  		# for i in range(0,n_nodes):
   			# X,Y,Z = data_nodal_disp_vec[i]
   			# vtkfile.write('%11.8e'%X+' '+'%11.8e'%Y+' '+'%11.8e'%Z+'\n')
  		# vtkfile.write("</DataArray>"+'\n')
    ...
    #     +" "+"Tensors="+'"'+""+'"'\
    #     +" "+"Vevtors="+'"'+""+'"'\
    #     +" "+"Scalars="+'"'+"Stress_Max_Principal"+'"'+">"+'\n')
    
    # # Boundary Beams
    # vtkfile.write("<"+"DataArray"+" "+"type="+'"'+"Float32"+'"'+" "+"Name="+'"'+"Boundary_flag"+'"'+" "+"format="+'"'+"ascii"+'"'+">"+'\n')
    # for i in range(0,ncell):
    #     X = boundary_flag_vtk[i]
    #     vtkfile.write('%11.8e'%X+'\n')
    # vtkfile.write('</DataArray>'+'\n')
    
    # # # Stress Max.Principal
    # # vtkfile.write("<"+"DataArray"+" "+"type="+'"'+"Float32"+'"'+" "+"Name="+'"'+"Stress_Max_Principal"+'"'+" "+"format="+'"'+"ascii"+'"'+">"+'\n')
    # # for i in range(0,n_elements):
    # #     X = data_element_maxps_scalar[i]
    # #     vtkfile.write('%11.8e'%X+'\n')
    # # vtkfile.write('</DataArray>'+'\n')
...
# =============================================================================
    ...
    vertpath = outpath.joinpath(geoName + '_vertices'+'.vtu')
    vertfile = vertpath.absolute().as_posix()

    vtkfile_vertices = open (vertfile,'w')
    #vtkfile_vertices = open (os.path.join(vtk_path,odbname)+"_instance"+str(ins_id)+step_name+"_"+frame_name+'.vtu','w')
...
# =============================================================================
    ...
    beampath = outpath.joinpath(geoName + '_beams'+'.vtu')
    beamfile = beampath.absolute().as_posix()

    vtkfile_beams = open (beamfile,'w')
...
# =============================================================================
    ...
    connspath = outpath.joinpath(geoName + '_conns'+'.vtu')
    connsfile = connspath.absolute().as_posix()

    vtkfile_conns = open (connsfile,'w')
    ...
    # for j, data_type_vector in enumerate(data_types_vector):
    #     vtkfile_conns_vol.write("<"+"DataArray"+" "+"type="+'"'+"Float64"+'"'+" "+"Name="+'"'+data_type_vector+'"'+" "+"NumberOfComponents="+'"'+"3"+'"'+" "+"format="+'"'+"ascii"+'"'+">"+'\n')
    #     for i in range(len(conn_connectivity)):
    #         X,Y,Z = conn_svars[2*i][(j)*3+6],conn_svars[2*i][(j)*3+7],conn_svars[2*i][(j)*3+8]
...
# =============================================================================
    ...
    outpath = Path(App.ConfigGet('UserHomePath') + '/woodWorkbench') 
    connvpath = outpath.joinpath(geoName + '_conns_vol.vtu')
    connvfile = connvpath.absolute().as_posix()
    vtkfile_conns_vol = open (connvfile,'w')
...
# class AbaqusGenerator:
#     def __init__(self,...)
        
        
def AbaqusFile(geoName,NURBS_degree,npatch,woodIGAvertices,beam_connectivity,\
               ...
               iprops_connector_t_top,props_connector_l,iprops_connector_l,\
               ...
               precrackFlag='off',precracked_elem=[]):
    # Beam
    ...
    outpath = Path(App.ConfigGet('UserHomePath') + '/woodWorkbench') 
    inppath = outpath.joinpath(geoName + '.inp')
    inpf = inppath.absolute().as_posix()

    inpfile = open(inpf,'w')
    # inpfile = open (os.path.join(path,geoName)+'.inp','w')
    inpfile.write('*HEADING'+'\n')
    inpfile.write('** Job name: {0} Model name: Model-{1}\n'.format(geoName,geoName))
    inpfile.write('** Generated by Wood Mesh Generator V1.0\n')
    ...
    inpfile.write(' PROPERTIES={:d}, INTEGRATION={:d}\n'.format(len(props_connector_t_bot),1))
    inpfile.write('1, 2, 3, 4, 5, 6\n')
    ...
    inpfile.write(' PROPERTIES={:d}, INTEGRATION={:d}\n'.format(len(props_connector_l),1))
    inpfile.write('1, 2, 3, 4, 5, 6\n')
    # beam element connectivity 
    count = 1
    ...
    inpfile.write('<VolumetricDeviatoricCouplingTConn>,<CompressiveUnloadingTConn>,<ConnectorTypeBot>,<NumberOfInstance>,<NumberOfBeamElem>,<NumberOfConnectorT>,<NumberOfConnectorTPrecrack>,<NumberOfConnectorL>,\n')
    inpfile.write('<NumberOfConnectorLPrecrack>\n')
    ...
    inpfile.write('<ShearStrengthRatioTConn>,<SofteningExponentTConn>,<InitialFrictionTConn>,<AsymptoticFrictionTConn>,<TransitionalStressTConn>,<TensileUnloadingTConn>,<ShearUnloadingTConn>,<ShearSofteningTConn>,\n')
    inpfile.write('<ElasticAnalysisFlagTConn>,<CompressiveYieldingStrengthTConn>,<InitialHardeningModulusRatioTConn>,<TransitionalStrainRatioTConn>,<DeviatoricStrainThresholdRatioTConn>,<DeviatoricDamageParameterTConn>,<FinalHardeningModulusRatioTConn>,<DensificationRatioTConn>,\n')
    inpfile.write('<VolumetricDeviatoricCouplingTConn>,<CompressiveUnloadingTConn>,<ConnectorTypeReg>,<NumberOfInstance>,<NumberOfBeamElem>,<NumberOfConnectorT>,<NumberOfConnectorTPrecrack>,<NumberOfConnectorL>,\n')
    inpfile.write('<NumberOfConnectorLPrecrack>\n')
    ...
    inpfile.write('<ShearStrengthRatioTConn>,<SofteningExponentTConn>,<InitialFrictionTConn>,<AsymptoticFrictionTConn>,<TransitionalStressTConn>,<TensileUnloadingTConn>,<ShearUnloadingTConn>,<ShearSofteningTConn>,\n')
    inpfile.write('<ElasticAnalysisFlagTConn>,<CompressiveYieldingStrengthTConn>,<InitialHardeningModulusRatioTConn>,<TransitionalStrainRatioTConn>,<DeviatoricStrainThresholdRatioTConn>,<DeviatoricDamageParameterTConn>,<FinalHardeningModulusRatioTConn>,<DensificationRatioTConn>,\n')
    inpfile.write('<VolumetricDeviatoricCouplingTConn>,<CompressiveUnloadingTConn>,<ConnectorTypeTop>,<NumberOfInstance>,<NumberOfBeamElem>,<NumberOfConnectorT>,<NumberOfConnectorTPrecrack>,<NumberOfConnectorL>,\n')
    inpfile.write('<NumberOfConnectorLPrecrack>\n')
    ...
    inpfile.write('<ConnectorTypeLConn>,<NumberOfInstance>,<NumberOfBeamElem>,<NumberOfConnectorT>,<NumberOfConnectorTPrecrack>,<NumberOfConnectorL>,<NumberOfConnectorLPrecrack>\n')
...
############################# Abaqus txt File #################################
    outpath = Path(App.ConfigGet('UserHomePath') + '/woodWorkbench') 
    igapath = outpath.joinpath(geoName + 'IGA.txt')
    igafile = igapath.absolute().as_posix()
    txtfile = open (igafile,'w')
...
def AbaqusModelInfo(nbeam_per_grain,fiberlength,NURBS_degree,nctrlpt_per_beam,\
                    ...
                    connector_t_reg_connectivity,connector_t_top_connectivity,\
                    connector_l_connectivity,AbaqusMeshData):
    ...
    for i in range(0,nelem_connector_t_bot):
        mass += density_connector_t_bot*props_connector_t_bot[3]*AbaqusMeshData[i,11]*np.linalg.norm(AbaqusMeshData[i,5:8] - AbaqusMeshData[i,8:11])
    # mass of reg transverse connector
    for i in range(nelem_connector_t_bot,nelem_connector_t_bot+nelem_connector_t_reg):
        mass += density_connector_t_reg*props_connector_t_reg[3]*AbaqusMeshData[i,11]*np.linalg.norm(AbaqusMeshData[i,5:8] - AbaqusMeshData[i,8:11])
    ...
    for i in range(nelem_connector_t_bot+nelem_connector_t_reg,nelem_connector_t_bot+nelem_connector_t_reg+nelem_connector_t_top):
        mass += density_connector_t_top*props_connector_t_top[3]*AbaqusMeshData[i,11]*np.linalg.norm(AbaqusMeshData[i,5:8] - AbaqusMeshData[i,8:11])
    # mass of longitudinal connector
    for i in range(nelem_connector_t_bot+nelem_connector_t_reg+nelem_connector_t_top,AbaqusMeshData.shape[0]):
        mass += density_connector_l*props_connector_l[3]*np.linalg.norm(AbaqusMeshData[i,5:8] - AbaqusMeshData[i,8:11])    
    return mass